...
function Testimonial({test}) {
  return (
    <motion.li className="bg-primary-1300 rounded-2xl px-8 py-10 max-xl:px-6 max-md:py-8"
    ...
    >
      <div className="flex gap-x-1 pb-8 max-md:pb-6 ">
        {Array.from({length: test.rating}).map(( _,i) => (
          <Star key={i} className="fill-primary-100" alt="Filled Start Icon" />
        ))}
        {Array.from({length: 5 - test.rating}).map(( _,i) => (
          <Star key={i}  alt="Filled Start Icon" />
        ))}
      </div>
